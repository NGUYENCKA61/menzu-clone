...
export function SoftwareDescription({
  ...
  /**
   * A spec table rather than a panel: rules only, no border and no fill.
   *
   * The box was the problem, not the width. A bordered card has to end
   * somewhere, and wherever it ended it was either narrower than every other
   * block in this section or several hundred pixels wider than its own longest
   * line — an empty band with a frame around it. With no frame there is
   * nothing to look empty, so the table can run the full width the prose does
   * and the leftover room is simply page.
   *
   * The label column is pinned at 220px rather than set as a percentage: a
   * percentage of the full width would push each answer nearly 400px from the
   * word it answers, which is where the eye starts reading across the wrong
   * row.
   */
  const factsCard = (
    <dl className="mt-4 border-t border-white/[0.07]">
      {REQUIREMENTS.map((f) => (
        <div
          key={f.label}
          className="grid grid-cols-1 gap-x-6 gap-y-1 border-b border-white/[0.07] py-3.5 sm:grid-cols-[220px_1fr]"
        >
