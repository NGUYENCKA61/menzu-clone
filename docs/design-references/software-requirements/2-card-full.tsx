...
export function SoftwareDescription({
   ...
   *
   * Full width, like every other block in this section, with the label column
   * pinned at 220px instead of a percentage. That pin is what makes the width
   * survivable: at 30% the labels would stretch to nearly 400px and strand
   * each answer half a screen from the word it answers, whereas a fixed column
   * keeps the pair together and lets the leftover room fall at the right edge,
   * where it reads as margin rather than as a gap inside the row.
   *
   ...
   */
  const factsCard = (
    <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.02] px-5 py-1.5 sm:px-6 sm:py-2">
      <dl>
        {REQUIREMENTS.map((f, index) => (
          <div
            key={f.label}
            className={`grid grid-cols-1 gap-x-6 gap-y-1 py-3.5 sm:grid-cols-[220px_1fr] ${
