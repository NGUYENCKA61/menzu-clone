 ...
 * VS"), which is exactly what the box searches.
 *
 * Pure, so it can be tested with a list of names and nothing else.
 */

/** "HACK ĐỘT KÍCH CFVN - CF4VN" → "ĐỘT KÍCH CFVN": the game, in the shop's capitals. */
export function shortGameName(categoryName: string): string {
  const core = categoryName
    .replace(/^\s*hack\s+/i, "")
    .split(/\s+[-–|(]|\s+mới\b/i)[0]
    .trim();
  return core.split(/\s+/).filter(Boolean).slice(0, 4).join(" ").toUpperCase();
}
...
/**
 * "Tìm: HACK ĐỘT KÍCH CFVN bản OBV, VS…". Without any edition in the names,
 * the first tool's own name stands in; with no tools at all, the game alone.
 */
